...
$sql = "INSERT INTO scenario01 (id, page, cookie_id) VALUES (ifnull((SELECT max(id)+1 FROM `scenario01` as sc WHERE cookie_id = '".$str."'),0), 'c', '".$str."')";
$conn->query($sql);
...
(SELECT count( distinct cookie_id)FROM `scenario01` WHERE UNIX_TIMESTAMP(datetime) > (select min(unix_timestamp(datetime)) from scenario01 where cookie_id = '".$_COOKIE[$cookie_name]."') and page ='c') as `cc` FROM `scenario01` as `a` 
where a.id >0 and UNIX_TIMESTAMP(datetime) > (select min(unix_timestamp(datetime)) from scenario01 where cookie_id = '".$_COOKIE[$cookie_name]."' and page ='c')";
